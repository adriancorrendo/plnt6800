...
```{r}
reg_fit <- lm(yield ~ nitro, data = data_corn)
summary(reg_fit)
# Comparing analysis of variance options
anova(reg_fit)
Anova(reg_fit, type = 2)
Anova(reg_fit, type = 3)
```

### Linear Regression, no-intercept

```{r}
reg_noint <- lm(yield ~ -1 + nf, data = data_corn)
summary(reg_noint)
...
```{r}
anova_fit <- lm(yield ~ nf + rep, data = data_corn)
Anova(anova_fit, type = 3)
Anova(anova_fit, type = 2)
```
...
emmeans(lm_05, pairwise ~ year:topo) %>% 
  cld(., level = 0.05, decreasing = FALSE)
```
### Alternative 
...
### Regression
-   **Intercept**: Baseline value of the dependent variable.

-   **Slope**: Change in response variable per unit increase in predictor.
